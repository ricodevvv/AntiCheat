...
public enum SubCategory {
	...
	private SubCategory(Category category, int slot, Material item) {
		...
		if (item == Material.DIAMOND) {
			item = katana.SERVER_VERSION.isNewerThan(ServerVersion.V_1_12_2) ? Material.CHEST_MINECART : Material.getMaterial("STORAGE_MINECART");
		} else if (item == Material.EMERALD) {
			item = katana.SERVER_VERSION.isNewerThan(ServerVersion.V_1_12_2) ? Material.BIRCH_STAIRS : Material.getMaterial("BIRCH_WOOD_STAIRS");
		} else if (item == Material.GOLD_INGOT) {
			item = katana.SERVER_VERSION.isOlderThanOrEquals(ServerVersion.V_1_7_10) ? Material.GLASS_BOTTLE : Material.getMaterial("RABBIT_FOOT");
		}
