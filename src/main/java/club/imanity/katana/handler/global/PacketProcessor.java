...
import club.imanity.katana.event.VelocityEvent;
import club.imanity.katana.event.WindowEvent;
import club.imanity.katana.handler.PlayerHandler;
...
import club.imanity.katana.util.Teleport;
import club.imanity.katana.util.TeleportPosition;
import club.imanity.katana.util.VersionBridgeHelper;
import club.imanity.katana.util.location.CustomLocation;
import club.imanity.katana.util.pending.BlockPlacePending;
import club.imanity.katana.util.player.BlockUtil;
import club.imanity.katana.util.set.PassiveExpiringSet;
...
public final class PacketProcessor extends SimplePacketListenerAbstract {
	...
	public void onPacketPlayReceive(PacketPlayReceiveEvent e) {
		...
		if (e.getUser() != null && e.getUser().getUUID() != null) {
			KatanaPlayer data = this.plugin.getDataManager().getPlayerData(e.getUser());
			...
			if (data != null) {
				...
				if (WrapperPlayClientPlayerFlying.isFlying(type)) {
					...
					if (katana.getInstance().getConfigManager().isAnticrash()
						&& katana.getInstance().getConfigManager().isLargeMove()
						&& (
					...
					handleOthers = false;
					if (katana.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19)) {
					...
					if (packet.getChannelName().equals("MC|Brand") || channelName.equals("minecraft:brand")) {
						...
						if (!brand.equalsIgnoreCase("vanilla") && !data.isBrandPosted() && katana.getInstance().getConfigManager().isClientCheck()) {
							data.setBrandPosted(true);
							MiscellaneousAlertPoster.postMisc(
								katana.getInstance().getConfigManager().getClientCheckMessage().replace("%player%", data.getName()).replace("%brand%", brand2), data, "Brand"
							);
						}

						String unallowed = katana.getInstance().getConfigManager().getConfig().getString("unallowed-brands.brands", "Vivecraft");
						if (unallowed.contains(brand)) {
							Tasker.run(
								() -> data.getBukkitPlayer()
										.kickPlayer(ChatColor.translateAlternateColorCodes('&', katana.getInstance().getConfigManager().getConfig().getString("unallowed-brands.kick-msg")))
							);
						}
					}
				}
				...
				if (!data.isRemovingObject()) {
					switch (packetID) {
						case ENTITY_EFFECT:
							...
							boolean via = katana.getInstance().isViaVersion();
						...
						case PING:
							...
							data.getThread().getExecutorService().execute(() -> katana.getInstance().getTransactionHandler().handleTransaction(id, nanoTime, data));
							handleAsync = false;
							break;
						case WINDOW_CONFIRMATION:
							...
							if (!transaction.isAccepted()) {
								data.getThread().getExecutorService().execute(() -> katana.getInstance().getTransactionHandler().handleTransaction(tid, nanoTime, data));
							}
	...
	public void handlePostPlayReceive(WrapperPlayClientPlayerFlying packet, KatanaPlayer data) throws CloneNotSupportedException {
		...
		if (!packet.hasPositionChanged()
			...
			&& katana.getInstance().getServerTick() - data.getCreatedOnTick() > 80L
			&& !data.isViaMCP()
			&& data.isNewerThan8()) {
			Tasker.run(
				() -> katana.getInstance()
		...
		if (!data.isCorrectedFly() && data.isInitialized()) {
			Tasker.run(() -> {
				if (katana.getInstance().getServerTick() - data.getServerTick() > 40L && !data.isAllowFlying() && data.getBukkitPlayer().getAllowFlight()) {
	...
	public void handlePlayReceive(PacketPlayReceiveEvent e, KatanaPlayer data, long nanoTime, long timeMillis) throws CloneNotSupportedException {
		Client type = e.getPacketType();
		if (data != null) {
			...
			katana.getInstance().getTransactionHandler().handlePlayReceive(e, nanoTime, data);
			if (isFlying) {
				...
				if (unload
					...
					if (katana.getInstance().getCheckState().isEnabled(flyA.getName())) {
				...
				if (katana.getInstance().getConfigManager().isPingKick()) {
					if (data.getTransactionPing() > (long) katana.getInstance().getConfigManager().getPingKickMaxPing()) {
						if (++data.badPingTicks >= katana.getInstance().getConfigManager().getPingKickTicks()) {
							Tasker.run(() -> data.getBukkitPlayer().kickPlayer(katana.getInstance().getConfigManager().getPingKickMsg()));
						}
					} else {
				...
				switch (type) {
					...
					case INTERACT_ENTITY:
						...
						boolean isPlayer = katana.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19) ? entity instanceof Player : EntityTypes.PLAYER.equals(entityType);
					...
					case PLAYER_DIGGING:
						...
						switch (dig.getAction()) {
							case START_DIGGING:
								...
								data.setDigTicks(data.getTotalTicks());
								if (katana.SERVER_VERSION.isOlderThanOrEquals(ServerVersion.V_1_12_2) && !data.isInUnloadedChunk()) {
									org.bukkit.Location blockIn = new org.bukkit.Location(data.getWorld(), position.getX(), position.getY(), position.getZ());
									Block block = katana.getInstance().getChunkManager().getChunkBlockAt(blockIn);
					...
					case USE_ITEM:
						if (type == Client.PLAYER_BLOCK_PLACEMENT && katana.SERVER_VERSION.isOlderThan(ServerVersion.V_1_9)) {
							...
							if (placed) {
								...
								Block blockAgainst = katana.getInstance().getChunkManager().getChunkBlockAt(against);
								...
								if (!invalid) {
									BlockPlacePending bpp = new BlockPlacePending(placedLocation, face.getFaceValue(), katana.getInstance().getServerTick(), itemInHand);
						...
						} else if (type == Client.USE_ITEM && katana.SERVER_VERSION.isNewerThan(ServerVersion.V_1_8_8)) {
						}
						break;
	...
	public void handlePacketPlaySend(PacketPlaySendEvent e, KatanaPlayer data, long nanoTime) {
		Server packetID = e.getPacketType();
		if (data != null) {
			Event callEvent = null;
			TransactionHandler network = katana.getInstance().getTransactionHandler();
			...
			switch (packetID) {
				...
				case PLAYER_POSITION_AND_LOOK:
					...
					boolean checkBB = katana.SERVER_VERSION.isOlderThanOrEquals(ServerVersion.V_1_7_10);
					double x = pos.getX();
