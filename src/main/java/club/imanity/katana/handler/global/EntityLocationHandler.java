...
public class EntityLocationHandler {
	public static void addEntity(KatanaPlayer data, EntityType entityType, double x, double y, double z, int eid) {
		EntityData ed = new EntityData(x, y, z, eid, entityType);
		data.entityData.put(eid, ed);
	}

	public static void updateEntityLook(KatanaPlayer data, int id) {
	...
	public static void updateEntityRelMove2(KatanaPlayer data, int id, double x, double y, double z) {
		EntityData edata = data.entityData.get(id);
		if (edata != null) {
			edata.newX += x;
			edata.newY += y;
	...
	public static void updateEntityTeleport2(KatanaPlayer data, int id, double x, double y, double z) {
		EntityData edata = data.entityData.get(id);
		if (edata != null) {
			edata.newX = x;
			edata.newY = y;
	...
	public static void updateEntityLocations(KatanaPlayer data) {
		data.entityData.values().forEach(EntityData::interpolate);
	}

	public static void updateFlyingLocations(KatanaPlayer data, WrapperPlayClientPlayerFlying flying) {
	...
	public static void destroyEntity(KatanaPlayer data, int[] id) {
		for (int a : id) {
