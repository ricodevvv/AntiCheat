...
import club.imanity.katana.handler.interfaces.KatanaHandler;
...
public final class CollisionHandler implements KatanaHandler {
	...
	public void handle(boolean moved) {
		...
		boolean newVer = katana.SERVER_VERSION.isNewerThanOrEquals(ServerVersion.V_1_13);
		double areaSize = this.data.elapsed(this.data.getUnderPlaceTicks()) <= 3 + MathUtil.getPingInTicks(this.data.getTransactionPing())
			? 2.0
			: Math.min(3.0, this.data.deltas.deltaXZ + 0.13);
		...
		KatanaStream<Material> blocksBelow = new KatanaStream<>(below.getCachedCollidingBlocks(tickCache));
		KatanaStream<Material> blocksNearHugeBelow = new KatanaStream<>(belowHugeBox.getCachedCollidingBlocks(tickCache));
		KatanaStream<Material> blocksInsideMiddleBox = new KatanaStream<>(inside.getCachedCollidingBlocks(tickCache));
		KatanaStream<Material> normalBox = new KatanaStream<>(this.data.getBoundingBox().getCachedCollidingBlocks(tickCache));
		KatanaStream<Material> lqStream = new KatanaStream<>(lqBelow.getCachedCollidingBlocks(tickCache));
		KatanaStream<Material> mcpLandStream = new KatanaStream<>(
			this.data
				.getBoundingBox()
				.clone()
		...
		KatanaStream<Material> mcpWaterStream = new KatanaStream<>(mcpWaterBox.getCollidingMaterialAccel(tickCache));
		KatanaStream<Material> mcpLavaStream = new KatanaStream<>(mcpLavaBox.getCollidingMaterialAccel(tickCache));
		KatanaStream<Material> blocksHori = new KatanaStream<>(horiBox.getCachedCollidingBlocks(tickCache));
		KatanaStream<Material> blocksAbove = new KatanaStream<>(above.getCachedCollidingBlocks(tickCache));
		KatanaStream<Material> blocksAboveStrict = new KatanaStream<>(aboveStrict.getCachedCollidingBlocks(tickCache));
		KatanaStream<Entity> entities = null;
		KatanaStream<EntityType> entities2 = null;
		if (katana.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19)) {
			entities = new KatanaStream<>(this.data.getBoundingBox().clone().expand(0.3, 0.6, 0.3).getCollidingEntities());
		} else {
			entities2 = new KatanaStream<>(this.data.getBoundingBox().clone().expand(0.3, 0.6, 0.3).getCollidingEntitiesNew());
		}

		KatanaStream<Entity> entitiesUp = null;
		KatanaStream<EntityType> entitiesUp2 = null;
		if (katana.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19)) {
			entitiesUp = new KatanaStream<>(this.data.getBoundingBox().clone().expandMin(0.0, 1.5, 0.0).expandMax(0.0, 0.7, 0.0).expand(0.65, 0.0, 0.65).getCollidingEntities());
		} else {
			entitiesUp2 = new KatanaStream<>(this.data.getBoundingBox().clone().expandMin(0.0, 1.5, 0.0).expandMax(0.0, 0.7, 0.0).expand(0.65, 0.0, 0.65).getCollidingEntitiesNew());
		}
		...
		boolean entity = katana.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19)
		...
		boolean livingEntity = katana.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19)
		...
		Block block = katana.getInstance().getChunkManager().getChunkBlockAt(climbable);
		Block sneakBlock = katana.getInstance().getChunkManager().getChunkBlockAt(this.data.getLastLocation().toLocation(this.data.getWorld()).add(0.0, -0.2, 0.0));
		...
		boolean boatsUp = katana.SERVER_VERSION.isOlderThan(ServerVersion.V_1_19)
		...
		Benchmark cacheData = KatanaBenchmarker.getProfileData(BenchmarkType.BLOCK_CACHE);
		cacheData.insertResult(nanoStart, nanoStartCollision);
		Benchmark profileData = KatanaBenchmarker.getProfileData(BenchmarkType.BLOCK_COLLISION);
	...
	public CollisionHandler(KatanaPlayer data) {
		this.data = data;
	}
}
