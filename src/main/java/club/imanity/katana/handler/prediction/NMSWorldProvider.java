...
package club.imanity.katana.handler.prediction;
...
import club.imanity.katana.util.mc.BlockPos.MutableBlockPos;
...
public final class NMSWorldProvider {
			...
			for (int k1 = i; k1 < j; ++k1) {
				for (int l1 = i1; l1 < j1; ++l1) {
					for (int i2 = k - 1; i2 < l; ++i2) {
						...
						Block block = club.imanity.katana.katana.getInstance().getChunkManager().getChunkBlockAt(loc);
						if (block != null) {
	...
	public NMSWorldProvider(club.imanity.katana.katana katana) {
		this.katana = katana;
	}
}
