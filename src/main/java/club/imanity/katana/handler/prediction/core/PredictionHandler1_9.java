...
package club.imanity.katana.handler.prediction.core;
...
public final class PredictionHandler1_9 extends AbstractPredictionHandler {
	...
	public PredictionHandler1_9(KatanaPlayer data, club.imanity.katana.katana katana) {
		super(data, katana);
	}
	...
	public void moveEntityWithHeading() {
		...
		if (!this.data.isOnWater()) {
			...
			float f2 = 0.02F;
			float f3 = 0.0F;
			boolean checkDepthStrider = club.imanity.katana.katana.SERVER_VERSION.isNewerThan(ServerVersion.V_1_7_10) && this.data.getBukkitPlayer().getInventory().getBoots() != null;
	...
	public void moveEntity(double x, double y, double z) {
		...
		if (this.data.isLastOnGroundPacket() && this.data.isSneaking()) {
			for (double d5 = 0.05;
				x != 0.0
					&& this.katana.getNmsWorldProvider().getCollidingBoundingBoxes(this.data.getBukkitPlayer(), this.getEntityBoundingBox().offset(x, (double)(-this.stepHeight), 0.0)).isEmpty();
				d2 = x
			) {
				double var20;
				double var21;
			...
			while (
				z != 0.0
					&& this.katana.getNmsWorldProvider().getCollidingBoundingBoxes(this.data.getBukkitPlayer(), this.getEntityBoundingBox().offset(0.0, (double)(-this.stepHeight), z)).isEmpty()
			) {
				double var24;
				double var25;
			...
			while (
				x != 0.0
					&& z != 0.0
					&& this.katana.getNmsWorldProvider().getCollidingBoundingBoxes(this.data.getBukkitPlayer(), this.getEntityBoundingBox().offset(x, (double)(-this.stepHeight), z)).isEmpty()
			) {
				double var22;
				double var23;
