...
public class EntityData {
	...
	public EntityData(double newX, double newY, double newZ, int eid, EntityType type) {
		...
		this.eid = eid;
		this.creationTime = katana.getInstance().getServerTick();
		this.type = type;
	}
	...
	public void postTransaction() {
		if (this.newLocations.size() > (katana.getInstance().getConfigManager().isReachSafe() ? 2 : 1)) {
			this.newLocations.remove(0);
		}

		this.uncertainBox = this.newLocations.size() > (katana.getInstance().getConfigManager().isReachSafe() ? 2 : 1);
	}
	...
	public long getExist() {
		return katana.getInstance().getServerTick() - this.creationTime;
	}
