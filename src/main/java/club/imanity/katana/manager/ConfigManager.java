...
public final class ConfigManager {
	...
	public ConfigManager(JavaPlugin katana) {
		this.loadConfig(katana, false);
		this.loadChecks(katana, false);
	}

	public void loadConfig(Plugin katana, boolean silent) {
		this.configFile = new File(katana.getDataFolder(), "config.yml");
		if (!this.configFile.exists()) {
			katana.saveResource("config.yml", false);
			if (!silent) {
				club.imanity.katana.katana.getInstance().printCool("&b| &fGenerating file &bconfig.yml");
			}
		} else if (!silent) {
			club.imanity.katana.katana.getInstance().printCool("&b| &fLoading file &bconfig.yml");
		}
		...
		if (!this.config.isSet("anticheat-name")) {
			this.config.set("anticheat-name", "Katana");
		}

		if (!this.config.isSet("server-name")) {
			this.config.set("server-name", "Katana");
		}
		...
		if (!this.config.isSet("banwaves.messages.complete")) {
			this.config.set("banwaves.messages.complete", "&bKatana &3has finished the banwave. A total of &b%bans% players &3were banned.");
		}
		...
		if (!this.config.isSet("katana-whitelist-msg")) {
			this.config.set("katana-whitelist-msg", "This server is whitelisted!");
		}
		...
		this.banwaveComplete = ChatColor.translateAlternateColorCodes(
			'&', this.config.getString("banwaves.messages.complete", "&bKatana &3has finished the banwave. A total of &b%bans% players &3were banned.")
		);
		...
		String acname = this.license.equals(" ") ? "VengeanceLoader" : "KatanaLoader";
		this.save();
	}

	public void loadChecks(JavaPlugin katana, boolean silent) {
		this.checkFile = new File(katana.getDataFolder(), "checks.yml");
		if (!this.checkFile.exists()) {
			katana.saveResource("checks.yml", false);
			if (!silent) {
				club.imanity.katana.katana.getInstance().printCool("&b| &fGenerating file &bchecks.yml");
			}
		} else if (!silent) {
			club.imanity.katana.katana.getInstance().printCool("&b| &fLoading file &bchecks.yml");
		}
		...
		if (!silent) {
			club.imanity.katana.katana.getInstance().getCheckState().initConfig(this.checks);
		}

		if (!this.firstTime) {
			club.imanity.katana.katana.getInstance().getCheckState().updateChecks();
		}
