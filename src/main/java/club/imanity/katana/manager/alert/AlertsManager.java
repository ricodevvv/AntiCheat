	...
	public void toggleAlerts(Player player) {
		if (!this.alertsToggled.remove(player.getUniqueId())) {
			...
			if (katana.getInstance().getConfigManager().isCrackedServer()) {
				Tasker.taskAsync(() -> katana.getStorage().setAlerts(player.getName(), 1));
			} else {
				Tasker.taskAsync(() -> katana.getStorage().setAlerts(player.getUniqueId().toString(), 1));
			}
		} else if (katana.getInstance().getConfigManager().isCrackedServer()) {
			Tasker.taskAsync(() -> katana.getStorage().setAlerts(player.getName(), 0));
		} else {
			Tasker.taskAsync(() -> katana.getStorage().setAlerts(player.getUniqueId().toString(), 0));
		}
	}
