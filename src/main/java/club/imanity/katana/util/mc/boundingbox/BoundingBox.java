...
package club.imanity.katana.util.mc.boundingbox;
...
import club.imanity.katana.handler.collision.CollisionBoxParser;
...
import club.imanity.katana.util.set.ConcurrentSet;
...
public final class BoundingBox implements Cloneable {
	...
	public BoundingBox(KatanaPlayer data, double x1, double y1, double z1, double x2, double y2, double z2) {
		...
		for (int i1 = i; i1 <= j; ++i1) {
			for (int j1 = k; j1 <= l; ++j1) {
				...
				if ((chunkPair = BlockUtil.getChunkPair(test)) != lastChunkPair) {
					katana.getInstance().getChunkManager().getChunk(test, c -> {
		...
		if (totalBlocks > 512) {
			...
			for (int x = xFloor; x <= xCeil; ++x) {
				for (int z = zFloor; z <= zCeil; ++z) {
					...
					boolean chunkLoaded = katana.getInstance().getChunkManager().isChunkLoaded(chunkLocation);
					if (chunkLoaded) {
						for (int y = yFloor - 1; y <= yCeil; ++y) {
							chunkLocation.setY((double)y);
							Block block2 = katana.getInstance().getChunkManager().getChunkBlockAt(chunkLocation);
	...
	public List<Block> getCollidingAir() {
		...
		for (int x = xFloor; x <= xCeil; ++x) {
			for (int z = zFloor; z <= zCeil; ++z) {
				Location chunkLocation = new Location(this.data.getWorld(), (double)x, 64.0, (double)z);
				Block block = katana.getInstance().getChunkManager().getChunkBlockAt(chunkLocation);
				if (block != null) {
