...
package club.imanity.katana.util.json;
...
import club.imanity.katana.util.BanData;
...
public class JsonReader {
	private static JsonObject readIp(String address) throws IOException {
		...
		request.setRequestProperty("User-Agent", "KatanaAC");
	...
	public static JsonObject sendBan(BanData ban) throws IOException {
		String sURL = "https://katana.cc/api/bandata";
		HttpsURLConnection request = (HttpsURLConnection)new URL(sURL).openConnection();
		request.setDoOutput(true);
		request.setRequestMethod("POST");
		request.addRequestProperty("User-Agent", "KatanaAC");
		...
		json.addProperty("katana_version", ban.katanaVer);
	...
	private static String[] getHaste(BanData data) {
		...
		if (vls.isEmpty()) {
			...
			StringBuilder end = new StringBuilder(
				"Anticheat logs for player " + data.player + " pasted with " + katana.getInstance().getConfigManager().getName() + " " + katana.getInstance().getVersion()
			);
