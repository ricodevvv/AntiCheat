...
import club.imanity.katana.util.Conditions;
...
public final class WorldChunkManager implements IChunkManager {
	...
	public Block getChunkBlockAt(Location location) {
		synchronized (this.loadedChunks) {
			...
			if (chunkMap.isEmpty()) {
				...
				Chunk chunk = (Chunk)chunkMap.get(BlockUtil.getChunkPair(location));
				if (chunk == null) {
					if (katana.getInstance().getServerTick() - this.lastAskTick >= 1L) {
						Tasker.run(() -> {
					...
					this.lastAskTick = katana.getInstance().getServerTick();
					return null;
				} else {
					int blockY = location.getBlockY();
					boolean invalidCoord = blockY > world.getMaxHeight() || blockY < 0;
					return katana.SERVER_VERSION.isNewerThanOrEquals(ServerVersion.V_1_13) && invalidCoord
						? location.getBlock()
	...
	private void somethingTriedDoingSomethingStupidErrorMessage(World world) {
		if (world == null) {
			Bukkit.getLogger().log(Level.SEVERE, "Katana attempted to access a chunk in a non-existent world, this should never happen null");
		} else {
			Bukkit.getLogger().log(Level.SEVERE, "Katana attempted to access a chunk in a non-existent world, this should never happen " + world.getName());
		}
	}
}
