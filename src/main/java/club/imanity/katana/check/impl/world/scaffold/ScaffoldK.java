...
public final class ScaffoldK extends PacketCheck {
	public ScaffoldK(KatanaPlayer data, club.imanity.katana.katana katana) {
		super(data, katana);
	}
...
@SneakyThrows
	public boolean isNotGroundBridging(Vector blockLoc) {
			...
			Block block2 = club.imanity.katana.katana.getInstance().getChunkManager().getChunkBlockAt(blockLoc.toLocation(this.data.getWorld()));
