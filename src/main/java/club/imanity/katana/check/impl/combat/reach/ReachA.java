...
package club.imanity.katana.check.impl.combat.reach;
...
import club.imanity.katana.util.mc.MovingObjectPosition;
...
public final class ReachA extends PacketCheck {
	public ReachA(KatanaPlayer data, club.imanity.katana.katana katana) {
		super(data, katana);
	}

	@Override
	public void handle(Event packet) {
		if (!club.imanity.katana.katana.SERVER_VERSION.isNewerThanOrEquals(ServerVersion.V_1_19)) {
					...
					for (int target : this.data.getLastTargets()) {
						...
						if (edata != null) {
							if (this.data.isNewerThan8()) {
								...
								if (dist > 4.0) {
									club.imanity.katana.katana.getInstance().printCool("&b> &fKatana USER: " + this.data.getBukkitPlayer().getName() + " is reaching far " + dist);
		...
		for (int target : this.data.getLastTargets()) {
			if (this.data.getGameMode() != GameMode.CREATIVE && !this.data.isRiding() && !this.data.isSpectating() && !this.data.isPossiblyTeleporting()) {
				...
				for (Vec3 rLook : rotationVectors) {
					for (double height : this.data.getEyePositions()) {
						...
						if (intercept != null) {
							...
							if (club.imanity.katana.katana.getInstance().getAlertsManager().hasDebugToggled(this.data.getBukkitPlayer()) && distance > 3.0 && distance != Double.MAX_VALUE) {
				...
				if (distance == Double.MAX_VALUE) {
					if (club.imanity.katana.katana.getInstance().getConfigManager().isCheckHitbox()) {
