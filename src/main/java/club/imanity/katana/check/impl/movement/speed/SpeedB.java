...
public final class SpeedB extends PositionCheck {
	...
	public SpeedB(KatanaPlayer data, club.imanity.katana.katana katana) {
		super(data, katana);
	}
	...
	public void handle(MovementUpdate e) {
		...
		if (!(this.data.deltas.deltaXZ < 0.2) && this.checkValid(velocity)) {
			...
			double tMult = Math.max(club.imanity.katana.katana.getInstance().getConfigManager().getSpeedBMult(), 1.0001);
