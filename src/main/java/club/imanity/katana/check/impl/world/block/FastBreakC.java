...
import club.imanity.katana.util.ReflectionUtil;
...
public final class FastBreakC extends PacketCheck {
	...
	public FastBreakC(KatanaPlayer data, club.imanity.katana.katana katana) {
		super(data, katana);
	}

	@Override
	public void handle(Event packet) {
		if (!club.imanity.katana.katana.SERVER_VERSION.isNewerThanOrEquals(ServerVersion.V_1_9)) {
			if (packet instanceof SwingEvent) {
				if (this.digStarted && this.block != null) {
					this.blockHardness = club.imanity.katana.katana.SERVER_VERSION.isNewerThanOrEquals(ServerVersion.V_1_16) ? 0.05F : ReflectionUtil.getBlockDurability(this.block);
				...
				Block block = club.imanity.katana.katana.getInstance().getChunkManager().getChunkBlockAt(blockLocation);
				if (block == null) {
					return;
				}

				if (club.imanity.katana.katana.SERVER_VERSION.getProtocolVersion() >= 47 && block.getType() == Material.BARRIER && this.data.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_7_10)) {
					return;
				}
				...
				switch (digType) {
					case START_DIGGING:
						...
						this.simulateDig(canBreak);
						this.blockHardness = club.imanity.katana.katana.SERVER_VERSION.isNewerThanOrEquals(ServerVersion.V_1_16) ? 0.05F : ReflectionUtil.getBlockDurability(block);
						this.curBlockDamage += this.toolDigEfficiency / this.blockHardness / (!canBreak ? 100.0F : 30.0F);
						break;
	...
	private boolean testWater(KatanaPlayer data) {
		...
		Block block = club.imanity.katana.katana.getInstance().getChunkManager().getChunkBlockAt(bukkitLocation);
		if (block == null) {
			return false;
	...
	private void simulateDig(boolean canBreak) {
		...
		if (this.data.getFatigue() != 0) {
			if (club.imanity.katana.katana.SERVER_VERSION.isNewerThanOrEquals(ServerVersion.V_1_8)) {
				float f1;
