...
public final class SpeedC extends PositionCheck {
	public SpeedC(KatanaPlayer data, club.imanity.katana.katana katana) {
		super(data, katana);
	}
	...
	public void handle(MovementUpdate e) {
		...
		if (!(this.data.deltas.deltaXZ < 0.1)
			...
			double tMult = Math.max(club.imanity.katana.katana.getInstance().getConfigManager().getSpeedCMult(), 1.0001);
