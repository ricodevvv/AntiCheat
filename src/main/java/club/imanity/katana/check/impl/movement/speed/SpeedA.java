...
package club.imanity.katana.check.impl.movement.speed;
...
public final class SpeedA extends PositionCheck {
	...
	public SpeedA(KatanaPlayer data, club.imanity.katana.katana katana) {
		super(data, katana);
	}
	...
	public void handle(MovementUpdate update) {
		...
		if (this.data.isUsingItem()) {
			tags.add("item-use");
			if (club.imanity.katana.katana.getInstance().getConfigManager().isFlagNoSlow() && this.data.getVelocityHorizontal() == 0.0 && this.data.getBukkitPlayer().getWalkSpeed() < 0.21F) {
		...
		if (diff > (double)movementSpeed * Math.max(club.imanity.katana.katana.getInstance().getConfigManager().getSpeedAMult(), 1.03)) {
			if (this.data.elapsed(this.data.getLastFlyTick()) > 5
				...
				if (!club.imanity.katana.katana.getInstance().getConfigManager().isFlagNoSlow()) {
					maxVL = 3.5;
				}

				boolean shouldFix = !this.data.isEating() && !this.data.isUsingItem() ? this.violations > 10.0 : this.violations > 15.0;
				if (club.imanity.katana.katana.getInstance().getConfigManager().isFixEat() && item && shouldFix && !this.data.isPendingBackSwitch()) {
